...
int main ( int argc, char *argv[] ) {
  ...
  for(i = 0 ; i < n_trains ; i = i + 3){
  	thread_tasks[i].train_id = i;
  	thread_tasks[i].loading_time = 500;
  	thread_tasks[i].exit_time = 2000000;
  	thread_tasks[i].priority = 0;
	...
	usleep(300000);
  	...
  	thread_tasks[i+1].train_id = i+1;
  	thread_tasks[i+1].loading_time = 500;
  	thread_tasks[i+1].exit_time = 100000;
  	thread_tasks[i+1].priority = 1;
  	thread_tasks[i+1].entry_controller = common_entry_controller;
  	thread_tasks[i+1].exit_controller = common_exit_controller;
  	pthread_create( &threads[i+1], NULL, simulate_train, (void*) &thread_tasks[i+1] );

	usleep(300000);  	

	thread_tasks[i+2].train_id = i+2;
  	thread_tasks[i+2].loading_time = 500;
  	thread_tasks[i+2].exit_time = 100000;
  	thread_tasks[i+2].priority = 0;
  	thread_tasks[i+2].entry_controller = common_entry_controller;
  	thread_tasks[i+2].exit_controller = common_exit_controller;
  	pthread_create( &threads[i+2], NULL, simulate_train, (void*) &thread_tasks[i+2] );
	usleep(5000000);
  }
