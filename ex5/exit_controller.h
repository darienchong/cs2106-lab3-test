...
typedef struct exit_controller {
    ...
    // Controls access to exit lane.
    sem_t exit_semaphore;
    
    // Controls access to priority array.
    sem_t priority_semaphore;
    
    // Number of priorities (0-indexed).
    int num_priorities;
    
    // Array to keep track of how many trains of each priority level are in the bay
    // and waiting to exit.
    int *num_trains_of_priority;
